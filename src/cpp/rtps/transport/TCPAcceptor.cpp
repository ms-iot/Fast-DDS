...
TCPAcceptor::TCPAcceptor(
        ...
        const Locator_t& locator)
    : acceptor_(io_service, asio::ip::tcp::endpoint(asio::ip::address_v4::from_string(interfaceName),
        IPLocator::getPhysicalPort(locator)))
    , locator_(locator)
    , io_service_(&io_service)
{
    endpoint_ = asio::ip::tcp::endpoint(asio::ip::address_v4::from_string(interfaceName),
