...
TEST(StatisticsDomainParticipant, CreateParticipant)
{
    ...
    EXPECT_EQ(history_latency_type, participant->find_type(network_latency_type.get_type_name()));

    EXPECT_NE(nullptr, participant->lookup_topicdescription(eprosima::fastdds::statistics::RTPS_LOST_TOPIC));
    EXPECT_EQ(history_latency_type, participant->find_type(rtps_traffic_type.get_type_name()));
    ...
    EXPECT_EQ(history_latency_type, participant->find_type(discovery_type.get_type_name()));

    EXPECT_NE(nullptr, participant->lookup_topicdescription(eprosima::fastdds::statistics::PHYSICAL_DATA_TOPIC));
    EXPECT_EQ(history_latency_type, participant->find_type(physical_data_type.get_type_name()));
